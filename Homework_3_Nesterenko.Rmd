...
sprintf("£ %.0f", 2024020.34221)

library(forcats)

x <- left_join(x = payments, y = members, by = c("member_id"="id")) %>% 
  group_by(name) %>% 
...
y %>%    
  ...
  mutate(name = forcats::fct_reorder(name, total_donations_000)) %>%
  
  ggplot(aes(y = name , x = total_donations_000)) +
  geom_col()
